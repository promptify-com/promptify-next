  ...
  return (
    <Box sx={{ position: 'relative', px: '16px' }}>
      <Box sx={{ position: 'sticky', top: 0, left: 0, right: 0, zIndex: 998 }}>
        {searchShown && (
          ...
          >
            ...
            <IconButton
              sx={{ border: 'none', bgcolor: 'surface.1', ':hover': { color: 'tertiary' } }}
        ...
        <Stack
          gap={2}
          sx={{ display: { xs: 'flex', md: 'none' }, width: 'fit-content', ml: 'auto', pt: '10px' }}
        >
          {!searchShown && (
            <IconButton
              sx={{ border: 'none', bgcolor: 'surface.1', ':hover': { color: 'tertiary' } }}
      ...
      <Box sx={{ mx: { xs: 0, md: '15px' } }}>
        {newExecutionData && (
          <ExecutionCardGenerated execution={newExecutionData} templateData={templateData} />
        )}
        ...
        {isFetching ? (
          ...
          filteredExecutions.map((execution, i) => {
            return <ExecutionCard key={i} execution={execution} templateData={templateData} />;
          })
        ) : (
          <Typography sx={{ mt: '40px', textAlign: 'center' }}>No executions found</Typography>
        )}
      </Box>
    </Box>
  );
